...
@section('content')
      ...
      <div  class="flex h-screen antialiased text-gray-800">
        <div id='contianer'  class="flex md:flex-row flex-col h-auto w-full overflow-x-hidden">
            <div class='md:hidden cursor-pointer flex content-start items-center fixed top-0  bg-white w-full z-20  cursor-pointer mr-auto ' onclick='showSidbar()'>
                ...
                @isset($currentConv)
                    <div class='ml-8 font-semibold'>
                        {{Str::title($currentConv->name)}}
                            <div>
                                @if (Cache::get('is_active-' . $currentConv->user_id))
                                        <div class=" pb-0.5 text-indigo-700 font-semibold" style="font-size: 10px">online</div>
                                    @else
                                        <div class=" pb-0.5 text-indigo-700 font-semibold" style="font-size: 10px">{{\Carbon\Carbon::parse($currentConv->last_seen)->diffForHumans()}}</div>
                                    @endif
                            </div>
                    </div>
                @endisset
               
                <svg class='ml-auto  my-4  mr-10 ' width="47" height="26" viewBox="0 0 47 26" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <!--! start sid bar` -->
            <div id="sidbar" class="md:transform -translate-x-96 transition-all duration-75 hidden md:flex fixed flex-col h-full  md:sticky top-0 py-8 pl-4 pr-4 md:w-64 w-full z-20  bg-white flex-shrink-0">
                ...
                <div
                    ...
                    <form id="add-conv" action="{{route('chats.add')}}" method="post" class="w-full h-full">
                        @csrf
                        <div class="h-10 w-8/12 inline-block">
                            <input 
                                type="text" 
                                name="email" 
                        ...
                        <button name="add-btn" type="submit" class="bg-indigo-700 text-white w-3/12 h-10 rounded-lg">add</button>
                    ...
                    <div class="flex flex-col space-y-1 mt-4 -mx-2  overflow-y-auto">
                        ...
                        @foreach ($conversations as $con)
                            <form action="{{route('home')}}" method="post" class="conversation w-full">
                                ...
                                <button id="conv-{{$con->conv_id}}"
                                    class="flex flex-row
                                            items-center w-full
                                            @if (isset($currentConv) && $currentConv->conv_id == $con->conv_id)
                                                bg-indigo-100 
                                            @else
                                                hover:bg-gray-100 
                                            @endif 
                                    ...
                                    <div class="flex relative items-center justify-center h-8 w-8 bg-gray-200 rounded-full font-bold">
                                        {{Str::upper($con->name[0])}} 
                                            ...
                                            <span id="unread-count" class="
                                                @if ($con->unread_count <= 0)
                                                hidden
                                                @endif
                                                absolute w-4 h-4 
                                                -bottom-1 left-4 
                                    ...
                                    @else
                                        <span class=" ml-auto  text-indigo-700 font-semibold" style="font-size: 10px">{{\Carbon\Carbon::parse($con->last_seen)->diffForHumans()}}</span>
                                    @endif
            ...
            <!--! start chat body -->
            <div class="flex flex-col flex-auto min-h-0 px-4 py-1 mt-16">
                <!--! sratr chat content -->
                <div class="flex flex-col  flex-shrink-0  rounded-2xl bg-gray-100   px-4 py-1" style="min-height: 60vh">
        ...
        <form id="notification"  
            ...
            class="cursor-pointer absolute top-4 -left-80 bg-indigo-500 text-white rounded p-2 w-64 h-18 z-40">
            @csrf
            <input type="hidden" name="conv_id" value="">
            <input type="hidden" name="name" value="">

            <div class="flex border-b-2 ">
                <div class="font-bold text-xm">New Message</div>
                <div id="notifi-close" class="z-50 ml-auto mt-1 text-xs">
                    <svg  width="10" height="10" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect y="33.234" width="47" height="4" rx="2" transform="rotate(-45 0 33.234)" fill="#ffffff" />
                            <rect x="2.82843" y="1.52588e-05" width="47" height="4" rx="2" transform="rotate(45 2.82843 1.52588e-05)"
                                fill="#ffffff" />
                    </svg>
                </div>
            </div>
            <div class="font-semibold truncate mt-1 w-full" style="font-size: 12px" >
                <span class="font-bold" id="name">Abubakr Alameri</span>: 
                <span class="truncate" id="message">Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque magnam, explicabo iusto exercitationem deleniti tempora pariatur facilis voluptates repellendus repellat.</span> 
            </div>
